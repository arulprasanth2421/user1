...
//	}
	...
	public String order(@PathVariable Integer prodid,@PathVariable Integer quantity) {
		try {
			...
			String orderDTO=template.getForObject("http://PROJECT"+"/add/products/"+prodid+"/"+quantity,String.class);
			return "Added successfully";
	...
	public List<ProductDTO> viewProductsonCategory(@PathVariable String category) {
			...
			System.out.println("before template");
			List<ProductDTO> prodDTO=template.getForObject("http://PROJECT"+"/products/category/"+category,List.class);
			System.out.println(prodDTO);			
			return prodDTO;
